classdef Case2 < BasicCase
    ...
    methods
        ...
        function regions = gen_solution_func(obj, Ln, Rn, Tn, Bn)
            ...
            % 构造符号求和
            % 沿 x 方向 A_z
            A_zx_expr = (obj.yt - y) * obj.c_0x + (y - obj.yl) * obj.d_0x + ...
                symsum( ...
            ...
            A_zx = symfun(A_zx_expr, [x,y]);
            A_zy = symfun(A_zy_expr, [x,y]);
            ...
            % 傅里叶系数积分
            c_0x_int = (1/obj.tau_x) * int((1/obj.tau_y) * obj.B, x, obj.xl, obj.xl + obj.tau_x);
            d_0x_int = (1/obj.tau_x) * int((1/obj.tau_y) * obj.T, x, obj.xl, obj.xl + obj.tau_x);
            c_hx_int = (2/obj.tau_x) * int(obj.B .* cos(obj.beta_h*(x - obj.xl)), x, obj.xl, obj.xl + obj.tau_x);
            d_hx_int = (2/obj.tau_x) * int(obj.T .* cos(obj.beta_h*(x - obj.xl)), x, obj.xl, obj.xl + obj.tau_x);
            e_ny_int = (2/obj.tau_y) * int(obj.R .* sin(obj.lambda_n*(y - yl)), y, yl, yl + obj.tau_y);
            f_ny_int = (2/obj.tau_y) * int(obj.L .* sin(obj.lambda_n*(y - yl)), y, yl, yl + obj.tau_y);
            
            % 符号方程封装
            %{
            obj.eq_c0x = c_0x_int == symfun(sym(['c0x' suffix]), [x y]);
            obj.eq_d0x = d_0x_int == symfun(sym(['d0x' suffix]), [x y]);
            obj.eq_c_hx = c_hx_int == symfun(sym(['c_hx' suffix]), [x y]);
            obj.eq_d_hx = d_hx_int == symfun(sym(['d_hx' suffix]), [x y]);
            obj.eq_e_ny = e_ny_int == symfun(sym(['e_ny' suffix]), [x y]);
            obj.eq_f_ny = f_ny_int == symfun(sym(['f_ny' suffix]), [x y]);
            %}
            % 返回 cell array
